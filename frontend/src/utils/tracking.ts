...
export const getTrackingUrl = (carrier: string, trackingNumber: string) => {
  ...
  switch (carrier) {
    case 'UPS': return `https://www.ups.com/track?loc=en_US&tracknum=${number}`;
    ...
    default: return `https://www.google.com/search?q=${encodeURIComponent(`${trackingNumber} tracking`)}`;
  }
};
